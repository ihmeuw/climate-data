...
chunks_path = "/mnt/share/erf/ERA5/merged_era5_with_stations/grid_setup/chunks_with_elevation_v3/"
...
geometry = [Point(xy) for xy in zip(grid_df['longitude_left'], grid_df['latitude_left'])]
grid_gdf = gpd.GeoDataFrame(grid_df, geometry=geometry)
...
for i in range(1,13):
    ...
    if (year == 2023) & (i > 8): # ERA5 data only available up to a certain date. No need to keep going.
        continue
    ...
    gdf_merged_era5 = gpd.sjoin_nearest(grid_gdf, gdf_era5, how="left", distance_col="distances_era5", lsuffix='grid', rsuffix='era5')
    ...
    gdf_merged_era5['total_days_in_year'] = pd.to_datetime(gdf_merged_era5['year'], format='%Y').dt.is_leap_year + 365
