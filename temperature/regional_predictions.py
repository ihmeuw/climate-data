...
list_of_files = [os.path.join(path_to_chunk, file) for file in os.listdir(path_to_chunk) if file.split('_')[-1].split('.')[0] == str(year)]
...
model = load('/mnt/share/code/nhashmeh/downscaling/temperature/validation_model_one_dummy_v3.joblib')
scaler = load('/mnt/share/code/nhashmeh/downscaling/temperature/validation_scaler_one_dummy_v3.joblib')
...
for day in unique_days:
    ...
    loaded_data_day.loc[loaded_data_day['longitude_st'] > 180, 'longitude_st'] = loaded_data_day.loc[loaded_data_day['longitude_st'] > 180, 'longitude_st'] - 360
    ...
    missing_lcz_values = pd.DataFrame(columns=[f'lcz_value_{i}' for i in range(18) if f'lcz_value_{i}' not in existing_lcz_columns])
    ...
    data_expanded[missing_lcz_values.columns] = data_expanded[missing_lcz_values.columns].fillna(0)

    # Create 'day_of_year_sin' and 'day_of_year_cos' features (these will replace day_of_year)
    data_expanded['day_of_year_sin'] = np.sin(2 * np.pi * data_expanded['day_of_year']/data_expanded['total_days_in_year']) 
    ...
    data_expanded = data_expanded[['latitude_st', 'longitude_st', 'elevation', 't2m', 'month', 'year',
       'total_days_in_year', 'day_of_year_sin', 'day_of_year_cos',
       'lcz_value_1', 'lcz_value_2', 'lcz_value_3', 'lcz_value_4', 'lcz_value_5', 'lcz_value_6',  'lcz_value_7',
       'lcz_value_8', 'lcz_value_9', 'lcz_value_10', 'lcz_value_11', 'lcz_value_12', 'lcz_value_13',
       'lcz_value_14', 'lcz_value_15','lcz_value_16', 'lcz_value_17']]
    ...
    print('Processing data for day of year ' + str(day) + ' (month, year: ' + str(month_store[0]) + ', ' + str(year) + ')')
    ...
    data_expanded_trim = data_expanded[['latitude_st', 'longitude_st', 'elevation', 't2m', 'month', 'year']].copy()
