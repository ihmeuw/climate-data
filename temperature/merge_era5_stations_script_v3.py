...
lcz_filepath = (
    "/ihme/homes/nhashmeh/downscale_temperature/climate_zones/lcz_filter_v2.tif"
)
...
with reo.open(lcz_filepath) as src:
    ...
    station_data = station_data.dropna(
        how="any", subset=["latitude", "longitude", "temp", "elevation"]
    )  # drop rows where there are no coords (data isn't always clean...)
    ...
    for i in range(1, 13):  # loop through each month
        ...
        gdf_station = gpd.GeoDataFrame(
            group_i, geometry=gpd.points_from_xy(group_i.longitude, group_i.latitude)
        )
        ...
        raster_values_and_coords = [
            (value, src.xy(*src.index(*coord)))
            for coord, value in zip(coords, src.sample(coords))
        ]
        ...
        gdf_station[["lcz_longitude", "lcz_latitude"]] = gdf_station[
            "lcz_coords"
        ].apply(pd.Series)
        ...
        lcz_points = gpd.GeoSeries(
            gdf_station.apply(
                lambda row: Point(row["lcz_longitude"], row["lcz_latitude"]), axis=1
            )
        )
        ...
        era5_data_month_df["longitude"] = era5_data_month_df["longitude"].apply(
        ...
        gdf_merged_final = gpd.sjoin_nearest(
            gdf_station,
            gdf_era5,
            how="left",
        ...
        gdf_merged_final = gdf_merged_final.loc[
            gdf_merged_final["time_st"] == gdf_merged_final["time_era5"]
        ]
        ...
        gdf_merged_final = gdf_merged_final.loc[
            gdf_merged_final.groupby(["latitude_st", "longitude_st", "time_st"])[
                "distances_era5"
            ].idxmin()
        ]
        ...
        gdf_merged_final["total_days_in_year"] = (
            gdf_merged_final["time"].dt.is_leap_year + 365
        )

        # drop columns not used
        gdf_merged_final.drop(
            columns=[
                "time",
                "time_st",
                "geometry",
        ...
        gdf_merged_final.to_feather(
            output_path + year + "_" + str(i) + "_era5_stations_lcz_merged.feather",
            index=False,
        )
